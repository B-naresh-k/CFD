 ...
 int main (){
    ...
    double error=1.0;
 	...
 	an=1.0/pow(dy,2);
	  ...
	  error=1.0;
	  FILE*file1;
	  file1=fopen("error_p2_jacobi.txt","w");
	  do
	  {
	  	for(i=0;i<m;i++)
	  	{
	    ...
	    for(i=1;i<(m-1);i++)
	  	{
	  		for(j=1;j<(n-1);j++)
	  		{
	  		tem_new[i][j]= (1.0/ap)*(-as*tem_old[i][j-1]-aw*tem_old[i-1][j]-ae*tem_old[i+1][j]-an*tem_old[i][j+1]);
		...
		FILE*file2;
		file2=fopen("stream_p2_jacobi.plt","w");
